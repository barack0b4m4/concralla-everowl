...
local panelWidth = paintAreaWidth + menuSize + marginOffset * 2
local panelHeight = paintAreaHeight + headerSize + marginOffset * 2
...
local paintAreaPosX = panelPosX + menuSize + marginOffset
...
local brushData = {
	isActive = false,
	barColor = tocolor(50, 179, 239, 230),
...
local panelMenu = {
	{"New", tocolor(217, 83, 79), "new"},
	{"Undo", tocolor(240, 173, 78, 230), "undo"},
	{"Redo", tocolor(240, 173, 78, 230), "redo"},
	{"Save", tocolor(21, 219, 114), "save"},
	{"Colorpicker", tocolor(50, 179, 239, 230), "palette", false,
		activeWhen = function()
	...
	{"Brush thickness", tocolor(50, 179, 239, 230), "brushsize", false,
		activeWhen = function()
	...
	{"Theme (dark/light)", tocolor(50, 179, 239, 230), "theme"},
	{"Saved graffitis", tocolor(21, 219, 114), "savedgraffitis", 10,
		activeWhen = function()
	...
	{"Load image from URL", tocolor(244, 184, 235), "url_image", 11,
		activeWhen = function()
...
addCommandHandler("gsize",
	function (command, size)
		if not tonumber(size) or tonumber(size) <= 0 or tonumber(size) > 5 then
			outputChatBox("#b0e56b[Usage]:#ffffff /" .. command .. " [Size (1 - 5)]", 255, 255, 255, true)
		else
			graffitiSize = tonumber(size)
			outputChatBox("#b0e56b[Graffiti]:#ffffff The graffiti size sucessfully changed to #e56b6b" .. graffitiSize .. ".", 255, 255, 255, true)
		end
	end
)

addCommandHandler("gadmin",
	function ()
		adminMode = not adminMode
	end
)

addCommandHandler("gclean",
	function ()
		graffitiCleanMode = not graffitiCleanMode
		
		if graffitiCleanMode then
			outputChatBox("#b0e56b[Graffiti]: #ffffffGraffiti cleaning mode is now enabled.", 255, 255, 255, true)
		else
			outputChatBox("#e56b6b[Graffiti]: #ffffffGraffiti cleaning mode is now disabled.", 255, 255, 255, true)
		end
	end
)
				...
				if deltaTime >= 3000 then
					if utf8.len(path) > 0 then
						...
						if fileExtension and fileExtension == ".png" or fileExtension == ".jpg" or fileExtension == ".jpeg" or fileExtension == ".bmp" then
							...
							outputChatBox("#32b3ef[Graffiti]: #ffffffYou started to download the selected image. Please wait the result!", 255, 255, 255, true)
						else
							outputChatBox("#e56b6b[Graffiti]: #ffffffThe selected path extension isn't valid!", 255, 255, 255, true)
						end
					else
						outputChatBox("#e56b6b[Graffiti]: #ffffffFirst add the image URL!", 255, 255, 255, true)
					end
				else
					outputChatBox("#e56b6b[Graffiti]: #ffffffPlease wait #e56b6b" .. 3 - math.floor(deltaTime / 1000) .. " second#ffffff to the next download.", 255, 255, 255, true)
				end
			else
				outputChatBox("#e56b6b[Graffiti]: #ffffffOne download is already in progress!", 255, 255, 255, true)
			end
		end
	end
)
...
addEventHandler("onClientReceiveDownloadedImage", getRootElement(),
	function (imagePixels, errno)
		if errno == 0 then
			...
			outputChatBox("#32b3ef[Graffiti]: #ffffffThe selected image successfully saved.", 255, 255, 255, true)
			outputChatBox("#32b3ef[Graffiti]: #ffffffYou can reach the image on the 'Saved graffitis' panel.", 255, 255, 255, true)
		else
			outputChatBox("#e56b6b[Graffiti]: #ffffffThe selected image not available!", 255, 255, 255, true)
		end
	...
	function (fromClient, pixels, data)
		...
		if graffitiFile then
			...
			if fromClient == localPlayer then
				...
				outputChatBox("#b0e56b[Graffiti]: #ffffffThe new graffiti spraying successfully completed.", 255, 255, 255, true)
			end
		end
	end
)
	...
	function (weapon, _, _, hitX, hitY, hitZ)
		...
		if lastDrawedPixels and not startedGraffiti.isActive and not startedGraffiti.actionDisabled and #drawingSteps >= 2 and getTickCount() >= nextSprayingAfterTick then
			if getTickCount() >= tryToSprayGraffitiTick then
				...
				outputChatBox("#32b3ef[Graffiti]: #ffffffYou started to spray a new graffiti!", 255, 255, 255, true)
			end
					...
					if loadedGraffitis[k].interior == playerInterior and loadedGraffitis[k].dimension == playerDimension then
						...
						if screenX and screenY then
							...
							if distanceBetweenGraffiti <= 15 then
								...
								local labelText = nameSplit[2] .. "\n" .. drawedTime .. ((not loadedGraffitis[k].isProtected and "") or "\n#ff0000PROTECTED")
								...
								local iconSize = 64 * distanceMultiplier
								local iconPosX, iconPosY = screenX - ((iconSize * 2) / 2), (labelPosY + labelHeight) + (iconSize / 2)
...
addEventHandler("onClientClick", getRootElement(),
	function (button, state)
		if adminMode or graffitiCleanMode then
			if selected3DButton then
				if button == "left" and state == "up" then
					if string.find(selected3DButton, "trash|") then
						...
						if serverGraffitis[fileName].isProtected then
							outputChatBox("#e56b6b[Graffiti]: #ffffffYou can't remove this graffiti because is protected.", 255, 255, 255, true)
							return
						end
					...
					elseif string.find(selected3DButton, "clean|") then
						...
						if not getPedOccupiedVehicle(localPlayer) then
							if not isTimer(cleaningGraffitiTimer) then
								if serverGraffitis[fileName].isProtected then
									outputChatBox("#e56b6b[Graffiti]: #ffffffYou can't clean this graffiti because is protected.", 255, 255, 255, true)
									return
								end
								
								setElementFrozen(localPlayer, true)
								triggerServerEvent("graffitiCleanAnimation", localPlayer)
								cleaningGraffitiTimer = setTimer(cleanGraffiti, graffitiCleanTime, 1, fileName)
							else
								outputChatBox("#e56b6b[Graffiti]: #ffffffYou are already have an active graffiti cleaning process.", 255, 255, 255, true)
							end
						else
							outputChatBox("#e56b6b[Graffiti]: #ffffffYou can't use this feature while sitting in a vehicle.", 255, 255, 255, true)
						end
					end
				end
			end
...
function cleanGraffiti(fileName)
	...
	cleaningGraffitiTimer = nil
	
	setElementFrozen(localPlayer, false)
	outputChatBox("#b0e56b[Graffiti]: #ffffffSuccessfully finished the graffiti cleaning.", 255, 255, 255, true)
end
...
function renderTheEditor()
	...
	if isCursorShowing() then
		...
		if panelIsMoving then
			...
			paintAreaPosX = panelPosX + menuSize + marginOffset
	...
	--** Background @ Title
	dxDrawRectangle(panelPosX, panelPosY, panelWidth, panelHeight + 40 + marginOffset, panelColors["main_" .. panelTheme])
	dxDrawRectangle(panelPosX, panelPosY, panelWidth, headerSize, panelColors["header_" .. panelTheme])
	
	if colorData.isActive then
		dxDrawText("Graffiti Editor - #ccccccColorpicker", panelPosX + marginOffset, panelPosY, panelPosX + panelWidth, panelPosY + headerSize, tocolor(255, 255, 255), 1.0, mainFont, "left", "center", false, false, false, true)
	elseif savedGraffitisMenu.isActive then
		dxDrawText("Graffiti Editor - #ccccccSaved graffitis", panelPosX + marginOffset, panelPosY, panelPosX + panelWidth, panelPosY + headerSize, tocolor(255, 255, 255), 1.0, mainFont, "left", "center", false, false, false, true)
	elseif loadImagePanel.isActive then
		dxDrawText("Graffiti Editor - #ccccccLoad image from URL", panelPosX + marginOffset, panelPosY, panelPosX + panelWidth, panelPosY + headerSize, tocolor(255, 255, 255), 1.0, mainFont, "left", "center", false, false, false, true)
	else
		dxDrawText("Graffiti Editor", panelPosX + marginOffset, panelPosY, panelPosX + panelWidth, panelPosY + headerSize, tocolor(255, 255, 255), 1.0, mainFont, "left", "center", false, false, false, true)
	end
	
	if not promptDialog.isActive and isCursorWithinArea(cursorX, cursorY, panelPosX + panelWidth - iconSize - marginOffsetHalf, panelPosY, iconSize, headerSize) then
		dxDrawImageSection(panelPosX + panelWidth - iconSize - marginOffsetHalf, panelPosY, iconSize, iconSize, 0, 0, 32, 32, "files/images/icons.png", 0, 0, 0, tocolor(212, 0, 40))
		selectedButton = "close"
	else
		dxDrawImageSection(panelPosX + panelWidth - iconSize - marginOffsetHalf, panelPosY, iconSize, iconSize, 0, 0, 32, 32, "files/images/icons.png")
	end
	...
	if savedGraffitisMenu.isActive and not loadImagePanel.isActive then
		if #savedGraffitis >= 1 then
			...
			dxDrawText("You don't have any saved graffitis.", paintAreaPosX, paintAreaPosY, paintAreaPosX + paintAreaWidth, paintAreaPosY + paintAreaHeight, tocolor(255, 255, 255), 1.0, mainFont, "center", "center")
		end
		
		--> Scrollbar
		if #savedGraffitis > savedGraffitisMenu.imagePerPage then
			dxDrawRectangle(paintAreaPosX + paintAreaWidth + marginOffsetHalf / 2, paintAreaPosY, 5, paintAreaHeight, panelColors["scrollbarBg_" .. panelTheme])
			dxDrawRectangle(paintAreaPosX + paintAreaWidth + marginOffsetHalf / 2, paintAreaPosY + (paintAreaHeight / #savedGraffitis) * savedGraffitisMenu.page, 5, (paintAreaHeight / #savedGraffitis) * savedGraffitisMenu.imagePerPage, panelColors["scrollbar_" .. panelTheme])
		end
	end
	...
	if loadImagePanel.isActive then
		if not loadImagePanel.resized then
			...
			dxDrawText("Loading in progress" .. dots, paintAreaPosX, paintAreaPosY, paintAreaPosX + paintAreaWidth, paintAreaPosY + paintAreaHeight, tocolor(255, 255, 255), 1.0, mainFont, "center", "center", false, false, false, true)
			...
			if not loadImagePanel.playerCanDownload then
				...
				dxDrawText("Downloading in progress" .. dots, paintAreaPosX, paintAreaPosY, paintAreaPosX + paintAreaWidth, paintAreaPosY + paintAreaHeight, tocolor(255, 255, 255), 1.0, mainFont, "center", "center", false, false, false, true)
	...
	for i = 1, #panelMenu do
		if panelMenu[i] then
			local row = panelPosY + headerSize + marginOffset + ((i - 1) * (iconSize + marginOffset))
			
			if panelMenu[i].activeWhen and panelMenu[i].activeWhen() then
				dxDrawImageSection(panelPosX + ((menuSize - iconSize) / 2), row, iconSize, iconSize, 0, (panelMenu[i][4] and panelMenu[i][4] - 1 or i) * 32, 32, 32, "files/images/icons.png", 0, 0, 0, panelMenu[i][2])
				
				if isCursorWithinArea(cursorX, cursorY, panelPosX, row, menuSize, iconSize) then
					selectedButton = "menu|" .. i .. "|not_selectable"
				end
			else
				if isCursorWithinArea(cursorX, cursorY, panelPosX, row, menuSize, iconSize) and not promptDialog.isActive then
					dxDrawImageSection(panelPosX + ((menuSize - iconSize) / 2), row, iconSize, iconSize, 0, (panelMenu[i][4] and panelMenu[i][4] - 1 or i) * 32, 32, 32, "files/images/icons.png", 0, 0, 0, panelMenu[i][2])
					selectedButton = "menu|" .. i
				else
					dxDrawImageSection(panelPosX + ((menuSize - iconSize) / 2), row, iconSize, iconSize, 0, (panelMenu[i][4] and panelMenu[i][4] - 1 or i) * 32, 32, 32, "files/images/icons.png")
				end
			end
		end
	end
	...
	if brushData.isActive then
		...
		local barY = paintAreaPosY + paintAreaHeight + marginOffset * 2 + 40 + ((30 - brushData.barHeight) / 2)
		local brushRemap = reMap(brushData.currentSize, brushData.minimumSize, brushData.maximumSize, 0, brushData.barWidth)
		
		dxDrawRectangle(panelPosX, paintAreaPosY + paintAreaHeight + marginOffset * 2 + 40, panelWidth, 30, panelColors["footer_" .. panelTheme])
	...
	if promptDialog.isActive then
		...
		dxDrawText(promptDialog.text .. "\n\n\nApply #32b3ef[ENTER]#ffffff - Cancel #32b3ef[BACKSPACE]", panelPosX, panelPosY, panelPosX + panelWidth, panelPosY + panelHeight, tocolor(255, 255, 255), 1.0, mainFont, "center", "center", false, false, false, true)
	end
end
			...
			elseif string.find(selectedButton, "menu|") then
				...
				if savedGraffitisMenu.isActive then
					if string.find(selectedButton, "select|") then
						...
						promptDialog.text = "Are you sure you are editing the selected graffiti?\nThis action overwrites the current drawing on the drawing surface."
						promptDialog.value = selectedButton
					elseif string.find(selectedButton, "remove|") then
						promptDialog.isActive = true
						promptDialog.text = "Are you sure you want to delete the selected graffiti?\nThis action can't undone."
						promptDialog.value = selectedButton
					end
				else
