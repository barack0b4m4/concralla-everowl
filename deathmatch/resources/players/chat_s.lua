    ...
    if messageType == 0 then
        ...
        for id, player in ipairs(getElementsByType("player")) do
            ...
            if getDistanceBetweenPoints3D(posX1, posY1, posZ1, posX2, posY2, posZ2) <= chatRadius then
                outputChatBox("" .. playerName .. ": " .. message .. "", player, 255, 255, 255)
            end
        end

    elseif  messageType == 1 then           -- get the chatting player's position
        local playerName = getPlayerName(source)
        local posX1, posY1, posZ1 = getElementPosition(source)
        ...
        for id, player in ipairs(getElementsByType("player")) do
            local posX2, posY2, posZ2 = getElementPosition(player)
            if getDistanceBetweenPoints3D(posX1, posY1, posZ1, posX2, posY2, posZ2) <= chatRadius then
                outputChatBox("**" .. playerName .. " " .. message .. "", player, 203, 195, 227)
            end
        end
    end
...
function localOOC(source, commandName, ...)
    ...
    local posX1, posY1, posZ1 = getElementPosition(source)
        ...
        for id, player in ipairs(getElementsByType("player")) do
            local posX2, posY2, posZ2 = getElementPosition(player)
            if getDistanceBetweenPoints3D(posX1, posY1, posZ1, posX2, posY2, posZ2) <= chatRadius then
                outputChatBox("((" .. playerName .. ": " .. stringWithAllParameters .. "))", player, 220, 220, 220)
            end
        end
end
