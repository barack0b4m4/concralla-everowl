function makeNPCEnterToVehicle(npc,vehicle,seat,maxtime)
...
function makeNPCExitFromVehicle(npc,maxtime)
	...
	if veh then 
		...
		px,py,pz = getElementPosition(npc)
	end
	local move = true
	if move then
		--setElementPosition(npc,x,y,z,false)
...
function makeNPCDriveToPos(npc,x,y,z,maxtime)
	...
	if move then
		...
		setElementPosition(car,x,y,z,false)
